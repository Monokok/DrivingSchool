...
namespace Autoschool.ViewModel
{
                ...
                return registerForTheCourseCommand ??
                  (registerForTheCourseCommand = new RelayCommand(obj =>
                  {
                      if (selected_courseDataGrid != null && selected_student != null)
                      {
                          ...
                          student_courses = new List<courseDTO>();
                          OnPropertyChanged("student_courses");
                          foreach (var item in student_invitations)
                          {
                              student_courses.Add(studentService.GetAllCourses().Find(cours => cours.id == item.group_id
                                  )
                                  );
                          }
                          ...
                          //student_invitations = studentService.GetAllInvitations(); //обновили список регистрированных курсов
                          //student_courses = new List<courseDTO>();//
                          //foreach (var item in student_invitations) //обновили список курсов у студента
                          //{
                          //    student_courses.Add(studentService.GetAllCourses().Find(cours => cours.id == item.group_id
                          //        )
                          //        );
                          ...
                          // courses.Remove(selected_courseDataGrid);
    ...
    public List<invite_courseDTO> student_invitations 
        ...
        public AppMainViewModel(IStudentService studentService)
        {
            ...
            //courses = studentService.GetAllCourses().Where(c => c.category_id == selected_category.id).ToList();
        }
    }
}
